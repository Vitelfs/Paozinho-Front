...
export function EditVenda() {
  ...
  useEffect(() => {
    if (venda && produtosDisponiveis.length > 0) {
      const itensCarregados = venda.item_venda
        .map((item) => {
          const produto = produtosDisponiveis.find(
            (p) => p.id === item.produto_id
          );

          // Se o produto não for encontrado, pular este item
          if (!produto) {
            console.warn(
              `Produto com ID ${item.produto_id} não encontrado na lista de produtos disponíveis`
            );
            return null;
          }
        ...
        .filter((item): item is ItemVenda => item !== null); // Remove itens nulos
