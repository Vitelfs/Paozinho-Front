...
import { updateVendasSchema } from "@/schemas/vendas.schema";
...
export function EditVenda() {
  ...
  const loadVenda = useCallback(async () => {
    ...
    try {
      ...
      // Configurar data da venda
      setDataVenda(new Date(vendaData.data_venda));
      setObservacoes(vendaData.observacoes || "");

      // Usar dados do cliente que já vêm na venda
      const clienteData: ClienteEntity = {
        id: vendaData.cliente.id,
        nome: vendaData.cliente.nome,
        contato: vendaData.cliente.contato,
        endereco: "", // Não temos esse campo na resposta da venda
        ativo: true, // Assumimos que está ativo
        createdAt: {
  ...
  // Carregar dados iniciais da venda
  useEffect(() => {
    loadVenda();
    loadClientes();
    loadProdutos();
  }, [loadVenda, loadClientes, loadProdutos]);
  ...
  // Carregar itens da venda quando venda é carregada
  useEffect(() => {
    if (venda && produtosDisponiveis.length > 0) {
      const itensCarregados = venda.item_venda.map((item) => {
        const produto = produtosDisponiveis.find(
          (p) => p.id === item.produto_id
        );
        if (!produto) {
          // Se o produto não for encontrado, criar um produto "fantasma" com os dados do item
          const produtoFantasma: ProdutoEntity = {
            id: item.produto_id,
            nome: item.produto.nome,
            descricao: `Produto ID: ${item.produto_id}`,
            preco_custo: 0,
            preco_minimo_venda: Number(item.produto.preco_minimo_venda),
            preco_revenda: Number(item.produto.preco_minimo_venda),
            categoria: {
              id: "unknown",
  ...
  const salvarVenda = useCallback(async () => {
    ...
    try {
      ...
      const vendaAtualizada: UpdateVendasEntity = {
      ...
      // Validação com Zod
      const validationResult = updateVendasSchema.safeParse(vendaAtualizada);
      ...
      await vendasService.updateVenda(id, vendaAtualizada);
  ...
  }, [id, clienteSelecionado, itensVenda, observacoes, dataVenda, navigate]);
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <Heading1 className="flex items-center gap-2">
              ...
              Editar Venda #{venda.id.slice(-8)}
            </Heading1>
            <Paragraph className="text-muted-foreground mt-2">
              Edite os dados da venda, cliente e produtos
            </Paragraph>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2 pt-4">
                  <Button
                    ...
                    className="flex-1"
                  >
                    {salvando ? "Salvando..." : "Atualizar Venda"}
                  </Button>
                </div>
              </CardContent>
            </Card>
